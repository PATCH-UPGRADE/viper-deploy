...
SEED_OUTPUT=$(podman-compose -f /srv/viper/compose-aws.yml exec -T viper npx tsx scripts/seed-remediation-metric.ts)
echo "${SEED_OUTPUT}"
...
echo ${VULNERABILITY_ID} > /srv/viper/RDT_VULNERABILITY_ID
